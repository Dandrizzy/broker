...
//  },
//  {
//   id: 9, name: 'Platinum Plan', period: '1', interest: 1.5, amount: 2500, type: 'Flexible'
//  },
// ];

const UserPlan = () => {
 const { fetch: fn } = useGetApi({ key: 'plans' });
 const { fetch: plans, isFetching } = useGet({ fn, key: ['plans'] });
   ...
   <div className=" text-slate-600">
    ...
    <div className=" grid gap-4 sm:grid-cols-2 px-4 lg:grid-cols-3">
     ...
     {plans.map((plan) => <div key={plan.id} className="border shadow-md max-w-sm">
      ...
      <div className=" bg-white pt-3 pb-6 flex justify-center">
       <InvestBtn key={plan.id} plan={plan} />
      </div>
     </div>)}
    </div>
